...
def get_hand_type(hand: Tuple[str, int]) -> Literal[6, 5, 4, 3, 2, 1, 0]:
    ...
    # remap Jokers
    # Jokers should be converted to the most common card type
    most_common_card = ("", 0)
    for char, occurence in occurences.items():
        if char != "J" and occurence > most_common_card[1]:
            most_common_card = (char, occurence)
    if "J" in occurences and hand[0] != "JJJJJ":
        occurences[most_common_card[0]] += occurences["J"]
        del occurences["J"]
    ...
    if card_dist == [5]:
        ...
        assert False, f"{hand=}, {card_dist=}"
    return hand_type.value


def get_card_rank(card: str, index: int) -> int:
    valid_cards = ["J", "2", "3", "4", "5", "6", "7", "8", "9", "T", "Q", "K", "A"]
    assert 0 <= index < 5
